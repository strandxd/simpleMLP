...
*/
MLP::MLP(std::vector<double> &input_row, bool default_init)
{
...
*/
void MLP::insert_sample(std::vector<double> &input_data)
{
    int total_input_nodes = map_of_layers[0].total_nodes;
    for (int i = 0; i < total_input_nodes; i++){
        if (i == total_input_nodes-1){ // Bias node
            continue; // Already sat output value when initializing nodes. Maybe a bit unclean
        }
        else{
